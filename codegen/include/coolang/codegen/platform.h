...
inline std::optional<std::filesystem::path> GetWindows10KitsPath() {
  HKEY roots_key;
...
inline std::optional<std::filesystem::path> GetLatestWindows10KitLibPath() {
...
inline std::optional<std::filesystem::path> GetMsvcInstallPath() {
...
inline std::optional<std::filesystem::path> GetMsvcToolsPath() {
...
inline std::optional<std::filesystem::path> GetMsvcLinkerPath() {
...
inline std::optional<std::filesystem::path> GetMsvcLibPath() {
  const auto msvc_tool_path = GetMsvcToolsPath();
  if (!msvc_tool_path.has_value()) {
    return std::nullopt;
  }
...
inline std::string GetLinkerCommandWindows(
  ...
  linker_cmd += "-LIBPATH:\"" + (win10_kit_lib_path.value() / "um/x86").string() + "\" ";
  linker_cmd += "-LIBPATH:\"" + (win10_kit_lib_path.value() / "ucrt/x86").string() + "\" ";
